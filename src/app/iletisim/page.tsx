    ...
    <>
      <div className="page-hero">
        <div className="container">
          ...
          <h1>Bizimle İletişime<br />Geçin</h1>
          <p style={{ marginTop:16 }}>Ücretsiz keşif ve fiyat teklifi için hemen ulaşın</p>
        </div>
      </div>
      ...
      <section className="section">
        <div className="container">
          <div className="contact-grid">
            ...
            {/* Left: Info */}
            <div>
              <h2 className="section-title reveal" style={{ fontSize:28, marginBottom:12 }}>İletişim Bilgileri</h2>
              <p className="reveal reveal-d1" style={{ color:'var(--aday-mute)', fontSize:15, lineHeight:1.7, marginBottom:32 }}>
                Bizi arayabilir, WhatsApp&apos;tan yazabilir veya formu doldurabilirsiniz.
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                {INFO.map((item, i) => (
                  <div key={item.title} className={`reveal reveal-d${i+1}`}
                    ...
                    <div>
                      <div style={{ fontWeight:700,fontSize:11,fontFamily:'var(--font-body)',marginBottom:4,color:'var(--aday-mute)',textTransform:'uppercase',letterSpacing:'0.08em' }}>{item.title}</div>
            ...
            <div className="reveal reveal-d1">
              {sent ? (
                <div style={{ background:'#F0FDF4',border:'1px solid #86EFAC',borderRadius:'var(--radius-lg)',padding:56,textAlign:'center' }}>
                  ...
                  <h3 style={{ fontFamily:'var(--font-heading)', fontSize:22, fontWeight:700, marginBottom:12 }}>Mesajınız İletildi!</h3>
                  <p style={{ color:'var(--aday-mute)', fontSize:15, lineHeight:1.6 }}>WhatsApp&apos;a yönlendirildiniz. En kısa sürede size dönüş yapacağız.</p>
                ...
                <form onSubmit={handleSubmit}
                  ...
                  <h3 style={{ fontFamily:'var(--font-heading)', fontSize:22, fontWeight:700, marginBottom:28 }}>Bize Ulaşın</h3>

                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
                    {([['name','Ad Soyad','Ad Soyad',true],['phone','Telefon','05xx xxx xx xx',true]] as const).map(([k, label, ph, req]) => (
                      <div key={k}>
                  ...
                  <div style={{ marginBottom:16 }}>
                    <label style={{ display:'block', fontSize:11, fontWeight:600, marginBottom:6, color:'var(--aday-mute)', fontFamily:'var(--font-body)', textTransform:'uppercase', letterSpacing:'0.08em' }}>Konu</label>
                    <select value={form.subject} onChange={e => update('subject', e.target.value)}
                      style={{ ...inputStyle, cursor:'pointer' }}>
                      <option value="">Konu Seçin</option>
                      <option>Yeni Sistem Kurulumu</option>
                  ...
                  <div style={{ marginBottom:28 }}>
                    <label style={{ display:'block', fontSize:11, fontWeight:600, marginBottom:6, color:'var(--aday-mute)', fontFamily:'var(--font-body)', textTransform:'uppercase', letterSpacing:'0.08em' }}>Mesajınız *</label>
                    <textarea required value={form.message} onChange={e => update('message', e.target.value)} rows={5}
                      placeholder="İhtiyacınızı ve adresinizi kısaca belirtin..."
